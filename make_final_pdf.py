...
def make(path):
    ...
    slide(c, 'Step 3: 프라이버시 역할 매핑 (16가지 역할)', [
        ...
        '--ARG0 → FIRST_PARTY_ENTITY (서비스 제공자: "We", "Company", "The Service")',
        '--ARG1 → DATA (수집되는 정보: "email", "location", "browsing history", "IP address")',
        '--ARGM-MNR → MECHANISM (수집 방법: "through cookies", "via API", "automatically")',
        '--ARGM-TMP → USER_TRIGGER (수집 조건: "when you use our service", "upon registration")',
        '--ARGM-PRP → PURPOSE (수집 목적: "to improve our services", "for analytics")',
        '>>ARGM-PRP: PropBank Purpose 수식어. "to provide better service"처럼 목적을 나타내는 절',
        '',
        '===16가지 프라이버시 역할 전체 목록 — 카테고리별',
        ...
        '-->>USER_MECHANISM: 사용자가 직접 데이터를 관리하는 방법. "email us", "settings page" 등',
        '',
        '===역할별 실제 예시 (원문 텍스트)',
        'DATA: "your name, email address, phone number, and payment information"',
        '--MECHANISM: "through the use of cookies, web beacons, and similar technologies"',
        '--THIRD_PARTY_ENTITY: "with our advertising partners, including Google and Facebook"',
        '--TIME_PERIOD: "for up to three years after your account is deleted"',
        '--CONSEQUENCE: "you may not be able to access certain features of our services"',
        '',
        '===같은 동사, 다른 카테고리 → 완전히 다른 매핑',
        '"share" + TPSC: ARG2→THIRD_PARTY_ENTITY / "share" + FPCU: ARG2→PURPOSE',
        '--이 차이가 자동화 불가 이유 → 146개 동사 전수 수동 매핑 테이블 구축 필요',
        '>>SHARING_TERMS: 데이터 공유 조건·방식 역할. "under strict confidentiality agreements" 등',
    ], 13)
    ...
    slide(c, 'Results: 대규모 분석 — 129,856개 정책의 완전성 점검', [
        ...
        '',
        '===카테고리별 정책 간 공출현 패턴 (Fig.2·Fig.3 연계 해석)',
        '대부분 정책은 FPCU + TPSC를 함께 기재 (공출현 0.64) → 수집하면 제3자 공유도 언급',
        '--UAED + DR 공출현 0.47: 삭제 언급 시 보존 기간도 같이 나오는 편 (논리적 연결)',
        '--CONSEQUENCE는 UCC와도 공출현 낮음 → 선택권 설명하면서도 결과는 빠뜨리는 사이트',
        '>>공출현(Co-occurrence): 두 항목이 같은 정책에 함께 등장하는 비율. 1.0=항상 함께 등장',
    ], 20)
    ...
    slide(c, '대규모 분석 해석 — 이게 왜 심각한 문제인가', [
        ...
        '',
        '===사용자 입장에서의 함의',
        '지금 당장 실천할 수 있는 것: 서비스 가입 전 USER_MECHANISM이 있는지 확인',
        '--"How to delete my account" 또는 "Contact us to request data deletion" 같은 문구 검색',
        '--CONSEQUENCE가 없으면 옵트아웃 결과가 불명확 → 어떤 효과를 기대할지 모름',
        '>>개인정보 자기결정권: 자신의 개인정보 수집·이용·제공 여부를 스스로 결정할 권리 (한국 헌법적 권리)',
        '',
        '===이 연구 이후의 흐름 — 2025년 현재',
        'GDPR 집행이 강화되면서 개인정보 자동 분석 도구에 대한 수요 증가 중',
        '--EU AI Act(2024 발효)와 개인정보 정책의 교차점: AI 시스템의 학습 데이터 투명성 요구',
        '--PolicyPulse 류의 도구가 AI 시스템 개인정보 감사에도 활용될 가능성 높음',
    ], 21, mode='interp')
    ...
    slide(c, 'ChatGPT-3.5 vs PolicyPulse — 같은 정책 요약해보니', [
        ...
        '',
        '===왜 법적 문서에서 환각이 특히 치명적인가',
        '"제3자에게 데이터를 공유하지 않습니다"라는 환각 → 사용자가 실제 공유가 있어도 모름',
        '--개인정보 처리방침은 법적 구속력 있는 문서 → 잘못된 요약이 법적 분쟁 원인이 될 수 있음',
        '--의료·금융 분야처럼 정확성이 요구되는 도메인일수록 구조적 추출의 가치가 높음',
        '>>법적 구속력(Legal Binding): 계약·약관·정책에 동의하면 그 내용을 따를 의무가 생기는 것',
        '',
        '===실용적 관점 — 어느 도구를 쓸 것인가',
        '빠른 일반 요약이 필요하다면: GPT-4 (자연스럽고 읽기 쉬움)',
        '--데이터 수집 역할의 정확한 추출·검증이 필요하다면: PolicyPulse (원문 근거 보장)',
        '--하이브리드: PolicyPulse로 구조화 추출 → LLM으로 자연어 설명 생성 (최상의 조합)',
    ], 22)
    ...
    slide(c, 'Conclusion — 논문이 남긴 것', [
        ...
        '--Weighted avg F1 0.97 / 모든 카테고리 precision·recall 0.92 이상 달성',
        '-->>절(Clause) 수준 분석: 문장 전체가 아닌 술어 중심 의미 단위(프레임)로 쪼개서 분석',
        '',
        '===핵심 기여 2 — 규모와 발견',
        '129,856개 정책 / 3,970만+ 프레임 처리 / 16가지 역할 / 5가지 응용 분야 제안',
        '--기존 SRL 기반 연구(PurPliance): 5개 정책 202문장 → 규모 차이가 압도적',
        '--50%+ 정책에서 UAED 완전 누락 / 75%에서 CONSEQUENCE 미기재 발견',
        '--규제 당국이 활용 가능한 대규모 정량적 증거를 최초로 제공',
        '>>Open Source: 코드·데이터 공개 → 재현 가능성 확보. github.com/crisp-du/ppevo',
        '',
        '===보고 나서 든 생각 — 솔직한 감상',
        '"SRL이 개인정보처리방침에 잘 맞나?" 처음에 의구심이 있었는데 읽을수록 맞았음',
        '--PropBank의 ARG0·ARG1·ARGM 구조가 "누가·무엇을·어떻게"와 거의 1:1 대응이라 딱 맞음',
        '--제일 인상적: UAED F1 0.15 → 0.92. 데이터 182건을 증강 + 2단계 구조로 극복한 것',
        '--근데 이게 개인정보처리방침에만 적용 가능한 건지, 다른 법적 문서에도 쓸 수 있는지 궁금함',
        '--한국어 정책에 적용하려면 결국 주석 데이터 구축이 가장 큰 벽인데 그게 현실적으로 어려울 듯',
        '',
        '===보안 공부 관점에서 본 시사점',
        '프라이버시 보안은 기술 취약점뿐 아니라 "정보 비대칭" 문제도 다뤄야 함',
        '--사용자가 데이터를 어떻게 다루는지 모르면 → 동의 행위 자체가 아무 의미 없음',
        '--PolicyPulse 같은 도구가 보급되면 개인정보 감사(audit)의 자동화가 현실화될 수 있음',
        '--실제 웹사이트 개인정보처리방침을 직접 확인할 때 이 논문의 분석 틀이 유용한 체크리스트가 됨',
        '',
        '코드·데이터 공개: github.com/crisp-du/ppevo  ·  NDSS 2025 채택',
    ], 25)
